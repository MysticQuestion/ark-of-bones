...
export default function EventsPage() {
  return (
    <>
      <SEO title="Events" description="Ark of Bones events, competition records, and confirmed dates." path="/events" image={eventHero} />
      <header className="record-mast">
        <div><p className="eyebrow">Events / 2026</p><h1>Calendar</h1></div>
      ...
      <section className="event-ledger" aria-labelledby="event-ledger-title">
        <div className="event-ledger__head"><p className="eyebrow" id="event-ledger-title">Schedule</p><span>Confirmed records only</span></div>
        <div className="event-ledger__empty">
          <span>—</span><strong>Public calendar open.</strong>
      ...
      <section className="record-gallery">
        <figure><img src={eventTable} alt="Ark of Bones table in active play" width="1400" height="700" loading="lazy" /><figcaption>Table / active play</figcaption></figure>
        <figure><img src={eventRacks} alt="Players using illuminated domino racks" width="1400" height="700" loading="lazy" /><figcaption>Rack detail</figcaption></figure>
      </section>
      <section className="record-index">
        <div className="record-index__label"><p className="eyebrow">Formats</p></div>
        <div className="record-index__rows">{formats.map((format,index)=><div className="record-index__row" key={format}><span>0{index+1}</span><strong>{format}</strong></div>)}</div>
      </section>
    </>
  );
}
